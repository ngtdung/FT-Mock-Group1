 ...
 *
 */
static IRQn_Type s_lpuartRxTxIrqId[LPUART_INSTANCE_COUNT] = LPUART_RX_TX_IRQS;
...
/**
 * @brief This static global is for init LPUART base
 *
 */
static void DRV_UART_initializeLpuartBase();
...
/**
 * @brief : This function is responsible for initialize LPUART base
 *
 */
static void DRV_UART_initializeLpuartBase()
{
    s_lpuartBase[0] = LPUART0;
    s_lpuartBase[1] = LPUART1;
    s_lpuartBase[2] = LPUART2;
}
...
DRV_UART_StatusType DRV_UART_Init(const DRV_UART_InstanceType instance, const DRV_UART_ConfigType *uartConfig)
{
    ...
    if (instance < DRV_UART_instanceCount && uartConfig != NULL)
    {
        ...
        LPUART_Type *base = s_lpuartBase[instance];
        s_UARTconfig[instance].bitCountPerChar = uartConfig->bitCountPerChar;
        s_UARTconfig[instance].parityMode = uartConfig->parityMode;
        s_UARTconfig[instance].stopBit = uartConfig->stopBit;
        s_UARTconfig[instance].baudRate = uartConfig->baudRate;
        s_UARTconfig[instance].transferType = uartConfig->transferType;
        s_UARTconfig[instance].clockSource = uartConfig->clockSource;
        ...
        // printf("\nINIT_stopbit = %x\n",(s_lpuartBase[instance]->BAUD & LPUART_BAUD_SBNS_MASK) >> LPUART_BAUD_SBNS_SHIFT);
        /*Set baudrate*/
