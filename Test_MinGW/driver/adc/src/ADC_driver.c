...
*
*/
ADC_Driver_ReturnCode_t DRV_ADC_Init(const uint8_t ADC_instance, ADC_Channel_type channel, IRQCallBack CallBackFunction)
{
    ADC_Driver_ReturnCode_t retVal = ADC_DRIVER_RETURN_CODE_ERROR;
    PDB_Type * PDBTarget = IP_PDB0;
    uint8_t PDBIndex = 0;
    ADC_Type *adcHwUnitId = NULL;
...
* @api
*/
ADC_Driver_ReturnCode_t DRV_ADC_EnableIRQ(uint8_t ADC_instance, ADC_Channel_type channel)
{
    ADC_Driver_ReturnCode_t retVal = ADC_DRIVER_RETURN_CODE_ERROR;
    IRQn_Type irqTarget;
    ADC_Type *adcHwUnitId = NULL;
    ...
    if((IP_ADC0 == adcHwUnitId))
    {
        retVal = ADC_DRIVER_RETURN_CODE_SUCCESSED;
        irqTarget = ADC0_IRQn;
    }
    else if((IP_ADC1 == adcHwUnitId))
    {
        retVal = ADC_DRIVER_RETURN_CODE_SUCCESSED;
        irqTarget = ADC1_IRQn;
    }
